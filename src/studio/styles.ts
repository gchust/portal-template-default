/**
 * Portal Studio — Shadow DOM stylesheet (Goal 05 maintainability split).
 * Moved OUT of the bootstrap (index.tsx): the dev-only injection mounts
 * this stylesheet INSIDE the shadow root, preserving Shadow isolation,
 * no host CSS leakage, theme behavior (via :host-context), and HMR
 * idempotence (the same <style> element is (re)created per mount).
 */
...
/** Detect the host's background luminance (light/dark palette). */
