...
- verified final tree rerun → PASS：`verified-final-*` typecheck、focused/full unit、Demo Chromium、built-in Chromium retry、build/check:package、fresh pack、frozen external consumer E2E/build/CLI 与 scans 均通过；fresh tarball SHA-256 恢复为指定 source tree 的 `d998dc1b6ac8205eaebfa9d6585a5c9a2db1de0de58b62a353913dcf4612f795`。built-in 首次 rerun 的 Portal popover click 偶发 FAIL 后原样 retry 3/3 PASS，失败与 retry 日志均保留在 `logs/verified-final-builtin-e2e*.log`。
