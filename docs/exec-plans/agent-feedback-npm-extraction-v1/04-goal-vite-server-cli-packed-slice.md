...
- [x] 2026-08-12 16:55 UTC — 从受控 fixture 新鲜复制到 repo 外 `/root/work/agent-feedback-g04-consumer-sYIC1p`，只在副本中指向已提交 HEAD 的外部 tarball；frozen install、browser/CLI/HMR、真实 SIGTERM、trace、build 与边界扫描全部通过，证据在 `/root/work/agent-feedback-g04-evidence-w3lfBr/`。
...
- [x] 2026-08-12 17:18 UTC — 独立复核按必须顺序复现 packed fixture 在 `test:e2e` 后无法执行 `agent-feedback list`：`shutdown.mjs` 删除整个 runtime root；改为仅删除 `session.json`，保留 active task，并单独提交 `a8b517917cf65d9b8ef80bd72d59c2fdf8dba226`。
- [x] 2026-08-12 17:22 UTC — 从 package HEAD `a8b517917cf65d9b8ef80bd72d59c2fdf8dba226` 的全新 archive 构建并打包，在 repo 外全新 consumer 按原始必须顺序全部通过；G04-001..G04-014 独立复核均为 PASS，最终证据在 `/root/work/agent-feedback-g04-independent-final2-IfEE2n/evidence/`，Goal 05–10 未开始。

### Surprises & Discoveries

- 受控的 `fixtures/packed-react-vite/` 是必须提交的静态 source/config/tests；lockfile、`node_modules`、`dist`、`.agent-feedback`、tarball、截图、trace 和日志仍只生成在 repo 外的新鲜副本与 evidence 目录。
- Vite 不重写 plugin-added inline module children 的 bare imports；完整 bootstrap 必须位于 resolved `virtual:agent-feedback/client` 中，HTML 只注入 `src="/@id/__x00__virtual:agent-feedback/client"` 的 module tag。packed Chromium 验证该根因修复。
...
- clean committed-HEAD archive 不含 ignored `dist`；原 focused test 的 CLI process tests 隐式依赖先前 build 产物，因此旧的通过结果不能证明文档规定的命令顺序。失败日志：`/root/work/agent-feedback-g04-independent-14QnhW/evidence/package-focused-tests-external.log`。
- 原 `shutdown.mjs` 用于隔离 SIGTERM 检查的全目录删除同时删除了 vertical test 留下的 active task，破坏后续必须 CLI 命令；session 生命周期检查只需删除 `session.json`。失败日志：`/root/work/agent-feedback-g04-independent-final-EkMCyW/evidence/44-required-cli-list.log`。

### Decision Log

- **D-04-01:** 提交 `fixtures/packed-react-vite/` 的静态 source/config/tests；每次验证从该目录复制一个 repo 外 consumer，只在外部副本中将 package specifier 改成 repo 外 tarball 绝对路径，并仅在外部生成 lockfile、`node_modules`、`dist`、`.agent-feedback`、截图、trace 和日志。
...
- **D-04-06:** resolved virtual module 是唯一 browser bootstrap module；`transformIndexHtml` 不含 inline children，只引用 Vite 的 encoded NUL `/@id/__x00__virtual:agent-feedback/client` URL。
...
- **G04-008 PASS** — `/root/work/agent-feedback-g04-evidence-w3lfBr/vertical-loop.png`、`packed-e2e-with-evidence.log` 与 trace 证明 browser annotation → active-task file → CLI complete/reopen → browser marker status。
...
- **G04-012 PASS** — fresh consumer 是 tracked fixture 的 repo 外副本；installed package 无 `src`，lock/modules scan 无 `link:`/`workspace:`/`/root/work/agent-feedback/` package source path（外部 tarball path 保留为预期的 `file:` resolution），解析路径位于 consumer `.pnpm/...tgz...`；仍通过 E2E/build/CLI。
...
- **fresh package archive:** `/root/work/agent-feedback-g04-independent-final2-IfEE2n/package-head/`。
- **fresh external consumer:** `/root/work/agent-feedback-g04-independent-final2-IfEE2n/consumer/`，从 tracked `fixtures/packed-react-vite/` 复制，只改外部副本的 tarball placeholder。
- **fresh evidence:** `/root/work/agent-feedback-g04-independent-final2-IfEE2n/evidence/`。
- **tarball:** `/root/work/agent-feedback-g04-independent-final2-IfEE2n/evidence/gchust-agent-feedback-0.1.0-alpha.0.tgz`；SHA256 `08dfe027878ac904dd28865746bd943fdaeb8228f226ed17ff2058bea0afc01e`。
...
- **G04-002 PASS** — tracked/external `main.tsx` 未注册 Agent Feedback；Chromium 看到自动注入的单一 `#agent-feedback-root`。
