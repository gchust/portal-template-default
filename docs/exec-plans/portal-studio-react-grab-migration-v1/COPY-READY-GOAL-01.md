# Copy-ready Goal 01 — Portal Studio React Grab single-engine migration

This file combines the shared migration contract, Goal 01 ExecPlan and copy-ready Codex commands. It is intended for an Agent that cannot conveniently read two repository files. The shared contract remains normative for every later Goal.

> Status (2026-08-11): Goal 01 is COMPLETE and committed on `feat-agent-feedback` under the user-approved amendment — `react-grab@0.1.50` is kept and G01-AC06 is revised to the deterministic semantic target-promotion rule in §3a. The Goal 01 ExecPlan (`01-goal-lock-upstream-contract.md`) records the amendment, evidence, and PASS results. Later Goals start from this amended contract.
...
The user explicitly approved continuing with `react-grab@0.1.50` and revised G01-AC06: clicking a nested SVG point must resolve to the meaningful interactive control rather than persisting the raw path, and the single perception path may use `getElementsAtPoint()`, `isElementGrabbable()`, DOM/composed-parent relationships and deterministic semantic rules to promote the hit to the nearest useful interactive target. See §3a for the normative rule.
...
- [ ] Inspect actual HEAD and baseline commands.
- [ ] Pin dependency and update lockfile.
- [ ] Add provenance note.
- [ ] Add dev/E2E-only fixture.
- [ ] Add contract tests.
- [ ] Run source/hit/freeze browser proof.
- [ ] Prove production exclusion.
- [ ] Independently audit G01-AC01…G01-AC14.
...
Fill at completion. Include PASS/FAIL/BLOCKED for every acceptance criterion and exact command results.
...
First run the preflight without changing code:

```text
/plan Read this complete document and inspect actual HEAD. Return a repository-adapted plan that maps every G01 acceptance criterion to concrete files, commands and evidence artifacts. Do not change code, do not widen scope, do not add compatibility/fallback, and do not start Goal 02.
```

Then activate the Goal:

```text
/goal Complete Portal Studio React Grab Migration Goal 01 from this document.

Inspect actual HEAD before changing code. Do not stop at a plan. Continue until every G01 acceptance criterion has concrete command/test/browser/build evidence. Keep Progress, Surprises & Discoveries, Decision Log, and Outcomes & Retrospective current. Report every criterion PASS, FAIL or BLOCKED. Do not switch production Portal Studio behavior and do not start Goal 02.
```
