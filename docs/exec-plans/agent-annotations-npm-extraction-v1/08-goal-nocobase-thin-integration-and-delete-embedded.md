...
Default Portal 从 packed `@gchust/agent-annotations` tarball 使用完整功能；模板只保留薄 NocoBase Extension、一次 Vite 注册和少量集成测试，原 `src/studio/**`、Studio scripts、通用测试和旧计划已物理删除。
...
7. 可选产品标题 “Portal Studio”，但通用包公共名称仍是 Agent Annotations。
...
agentAnnotations({
  ...
  allowRemote: env.NOCOBASE_AGENT_ANNOTATIONS_ALLOW_REMOTE === "true",
});
```
...
- **G08-011** package CLI 操作 Default Portal `.agent-annotations` 成功。
- **G08-012** Agent complete 后浏览器同步状态。
- **G08-013** Default Portal production build 无 Agent Annotations client/endpoints。
...
pnpm exec agent-annotations print --markdown
...
rg -n "@nocobase|data-nb-|data-ai-page-element|NOCOBASE_" ../agent-annotations/src ../agent-annotations/dist
```
...
- [x] 2026-08-13：切换 Vite plugin、CLI aliases 和 `.agent-annotations` runtime，删除 embedded Studio、旧 scripts/tests/plans。
...
- 使用 `annotations:*` CLI aliases 直接调用 package binary；不保留旧 scripts、aliases 或 `tsx`。
...
- 最终 package tarball：`/root/work/agent-annotations-g08-final-reaudit-20260813/artifacts/gchust-agent-annotations-0.1.0-alpha.0.tgz`，SHA-256 `06bbd8a85a4a2fa3ce346617e1d91f594e082540cdd6558bfa3c3af6cddd2276`。
- 首次审核报告：`/root/work/agent-annotations-g08-final-audit-20260813-6Qiawo/FINAL-AUDIT.md`（15 PASS / 1 FAIL）；最终 fresh 复审：`/root/work/agent-annotations-g08-final-reaudit-20260813/FINAL-AUDIT.md`（16 PASS / 0 FAIL）。
...
- `pnpm exec agent-annotations list` 和 `pnpm exec agent-annotations print --markdown` → PASS。
...
- G08-005 PASS — Vite 仅从 package import 并调用一次 `agentAnnotations(...)`。
...
- G08-011 PASS — installed package CLI 可读取 Default Portal `.agent-annotations`。
