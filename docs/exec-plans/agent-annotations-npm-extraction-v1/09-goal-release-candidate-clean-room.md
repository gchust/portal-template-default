...
```text
artifacts/
├── @gchust-agent-annotations-<version>.tgz
├── SHA256SUMS
├── PACK-CONTENTS.txt
...
- [x] 2026-08-13: package Goal 09 逻辑改动已本地提交为 `4dc320d599805ea44b30dea1deb1d78886fdfe44` (`chore: prepare Agent Annotations release candidate`)；从该候选 commit 启动 clean archive 矩阵。
...
- [x] 2026-08-13: 仅从 Node 22 clean archive 产出一个 final RC tarball：`artifacts/gchust-agent-annotations-0.1.0-alpha.0.tgz`，41 files / 135315 bytes / SHA-256 `241a7b60691dc3725869906cf710ee71ec1f4d768a8c0ce0a31fc92995fc6626`。
- [x] 2026-08-13: 首个 packed blank consumer 完整 browser/source/reliability/build/import/CLI 门禁 PASS，但发现 packed `agent-annotations audit` 无法在无 package `src` 的消费者运行；修复及回归测试已单独提交为 `688e960604f54c435b2564feedce7a0bed96d81f` (`fix: support audits from packed consumers`)。
...
- [x] 2026-08-13: `/root/work/agent-annotations-g09-writer-20260813/artifacts` 仅保留一个 RC tarball及 `SHA256SUMS`、`PACK-CONTENTS.txt`、release notes、writer acceptance report；25 files / 53051 bytes / SHA-256 `fbd9de0d4e81f738eff9a20b632cce348476ab4f8d0595631c04c87ea2701c54`，checksum 与精确 manifest PASS。
...
- [x] 2026-08-14: 在真实 `nb portal dev main` 的 `/x/main/` base 下复现工具栏缺失；HTML 将虚拟客户端硬编码为根路径 `/@id/...`，真实 Chromium 得到 404，`#agent-annotations-root=0`。
- [x] 2026-08-14: 通用 package 在 `8da19fbd34e94f4eb41fce933e37651dc21680a2` 修复 Vite resolved base 注入并增加回归测试；typecheck、21 files / 93 tests 和 build PASS。
...
- packed blank consumer 运行 `pnpm exec agent-annotations audit` 具体 FAIL：audit 无条件要求 package source 中的唯一 primitives importer，但 publish tarball 按合同不含 `src`;导致 audit 不可从 packed consumer 使用。
...
- 2026-08-13: 所有生成 consumer、tarball、日志、报告和截图都放在 `/root/work/agent-annotations-g09-writer-20260813`，两个仓库只保留需要版本化的实现、测试和文档。
