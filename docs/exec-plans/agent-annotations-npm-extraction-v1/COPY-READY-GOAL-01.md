...
/goal Establish a two-repository workspace with `agent-annotations/` and
`portal-template-default/`, create an independently buildable and packable
`@gchust/agent-annotations` skeleton, and record a fresh baseline of the current
...
1. `agent-annotations/` is an independent Git repository next to the current
...
2. Create package metadata, ESM-only multi-entry tsdown build, declarations,
   exports for `.`, `/vite`, `/extension`, `/types`, and an `agent-annotations`
   CLI bin.
