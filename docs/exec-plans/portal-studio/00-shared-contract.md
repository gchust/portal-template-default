...
| D-031 | Post-07 (real user feedback) | LAN access fixes for the Studio: (1) the loopback-only endpoint check rejected the dev machine's own LAN origin (192.168.2.199) → all endpoints 404; now trusted-source = loopback ∪ own interface addresses, with an `allowRemote` opt-in for other machines (still token-protected); (2) saveTask stranded in "Saving task…" on plain-http LAN origins because `crypto.randomUUID` is unavailable in non-secure contexts → `newTaskId()` fallback; (3) the fixed indigo toolbar clashed with the monochrome AI portal → theme-adaptive palette (light/dark detection via html→body→#root luminance incl. oklch/hsl) + host `--primary`/`--primary-foreground` inheritance for the primary action + theme-adaptive floating toggle; (4) "Unable to save task: not_found" from a remote LAN browser (source check) → dogfood enables `allowRemote` (dev workflow; token still required) and the error message now explains both stale-session and untrusted-origin causes | theme-save.test.ts (10 tests) + abuse.test.ts trusted-source cases; evidence-alpha-acceptance/d031-lan-*.png; verified live: endpoints 200/400-invalid_task (source check passed), "Task saved", light panel + white toggle |
| D-032 | Goal 00 (Annotation-first plan) | Reconcile + plan scope: authorized `git merge --ff-only origin/develop` in /root/work/nocobase (feat-agent-feedback 0 ahead / 7 behind → fast-forward 8cc448410b → f0a480e9f0; only substantive delta #10315 `nb portal push` local Git identity; user dirty states preserved in user's own stash@{0}, untouched); portal-template-default baseline = aeafbbb (clean, == origin/feat-agent-feedback, 10 commits; D-031 changes live inside aeafbbb "chore: test" — not rewritten); plan 08-annotation-first.md defines five sequential goals (G01 draggable dock, G02 persistent per-comment annotations schemaVersion 5, G03 per-marker edit-delete, G04 Copy/Complete/shared formatter, G05 hardening) with explicit exclusions (multi-task history/archive, commit rewriting, upstream rebase/merge, release, dogfood) — Goal 00 ships plan docs + review + local checkpoint only | evidence-reconcile/01-nocobase-merge.txt + 02-grounding.md; test baselines re-verified at aeafbbb (unit 224, e2e 6/6) |
| D-033 | Annotation-first (authoritative) | Product decision superseding plan details: replace fixed emoji toggle + large idle panel + single top-level instruction with Annotation-first — draggable dock with persisted position; collapsed = compact icon toolbar; single-element CONTINUOUS annotation (each pick = new annotation, no replace); TRUE multi-select mode; area annotation; per-annotation comments; numbered markers persisting across collapse/refresh/routes and FOLLOWING targets (re-resolved on render/navigation); unresolved targets retained (never dropped); per-marker edit/delete; hide-without-delete; Copy first-class and non-clearing; explicit Complete = only normal clear path; schemaVersion 5 with `annotations[]`; browser and CLI share one Markdown formatter; MCP stays non-crashing with NO scope expansion; NO large migration framework for unpublished v4 intermediates (simple normalize-on-read only) | 08-annotation-first.md "Revision 1" section; plan goal deltas G01(2,3) G02(4,7,8,9,14,17) G03(5,10,11) G04(12,13,15,16) |
| D-034 | Annotation-first (interaction) | Interaction contract layered on D-033: Enter = newline, Ctrl/Cmd+Enter = save (every comment editor); capture failure NEVER loses an annotation (task write decoupled from evidence capture, non-blocking notice); Studio UI cannot annotate itself (isStudioElement extends to picking/marquee/marker re-resolution/hit-testing; overlay outside the screenshot DOM clone); deleting item N RENUMBERS displayed markers (display = live 1-based order index of annotations[], stable annotationIds unchanged); toolbar uses lucide-react (^0.487.0, already a dependency) with NO emoji glyphs; global destructive actions under "More" (⋯); Clipboard fallback = manual copy textarea; route/scroll/resize/HMR/a11y/security/production-exclusion are ACCEPTANCE concerns for the whole feature | 08-annotation-first.md "Revision 2" section; lucide-react verified in package.json devDependencies |
| D-035 | Annotation-first (licensing) | Licensing constraints: Instruckt (MIT) may be selectively adapted WITH attribution, but only when code is copied (adapted files carry the MIT notice + copyright header + provenance note); Agentation (PolyForm Shield) is design-reference ONLY — its source must never be copied/adapted/line-for-line reimplemented, only observable UX patterns may inform the design; applies to all goals G01–G05; borrowed-design provenance recorded per goal | 08-annotation-first.md "Revision 3" section |
| D-036 | Annotation-first (scope boundary) | OUT OF SCOPE for G01–G05: hosted AI employees; browser source editing; backend data mutation (no NocoBase collection/data writes from the Studio; only dev-only `.portal-studio/` artifacts persist); new MCP features (existing five tools unchanged, only non-crashing v5 rendering); unrelated refactors (AGENTS.md focus rule) | 08-annotation-first.md "Revision 4" section |
| D-037 | G01 (dock) | Dock persistence & placement: key `portal-studio.dock` = JSON `{x,y}` (top-left anchor of the dock row); invalid/missing values fall back to the default bottom-right anchor (16px margin); storage failures (privacy mode) degrade silently; the row width is measured once on mount (fallback constant in jsdom) so clamping keeps the whole row visible; drag = window-capture pointer listeners with a 4px threshold (click vs drag disambiguated via a per-gesture flag, no pointer capture so button clicks stay natural); keyboard = arrows (Shift = larger step); the expanded panel anchors ABOVE the toggle (bottom = toggle top − 8) and flips BELOW when less than 200px of space remains above; More (⋯) menu hosts global destructive actions (Reset dock position now; Clear all annotations gated for G03) | dock.test.ts (17 tests: clamp/move/persistence/layout), toolbar.test.tsx (18), e2e dock step (drag → reload → retained); evidence-annotation-first-g01/ |
| D-038 | G02 (persistent annotations) | Marker re-resolution strategy + v5 normalize details: markers re-resolve via the FIRST matching selector candidate of each element capture (id → data-ai-page-element → role → bounded path), on render and on ANY body-subtree mutation (debounced 80 ms — async table loads, route swaps, HMR), scroll, and resize; unresolved targets are RETAINED in the list (grey "Target not found") with no page anchor; markers render INSIDE the shadow host (D-034 #3 mounting rule) so they never pollute screenshots and can never be annotated; v4→v5 normalize-on-read maps instruction→single annotation comment + elements→captures + region→rect (annotationId `${taskId}-v4`); per-annotation element cap = existing MAX_ELEMENTS (totals bounded by the 256 KB artifact cap — the initial global sum cap conflicted with accumulated annotations and was removed after the e2e found it); kind = region when a region exists, else element/multi by element count | task-model.test.ts, endpoint.test.ts (v4/v5/v1), print-cli v5/v4 rendering, mcp parity, e2e "annotations: continuous picks…" (reload persistence, route-follow, unresolved retention, Ctrl+Enter); evidence-annotation-first-g02/ |
| D-039 | G03 (multi-select & per-marker ops) | Multi-select merge semantics + hide-vs-delete UX + empty-task validity: TRUE multi-select = a distinct mode where a seed pick starts ONE group and clicks/Space toggle elements in/out of the SAME annotation (bounded MAX_GROUP_ELEMENTS=50, local to task-model so it stays vite-config-load-safe per D-008), Enter finishes into the group comment editor, Esc cancels; per-marker ops are pure functions over annotations[] (remove/toggle hidden/update comment) with live display renumbering (D-034 #4) and stable annotationIds; hide sets `hidden:true` (persists, badge/anchor hidden, list dimmed) WITHOUT deletion; deletions use an inline accessible confirmation (role=alert, focusable, Esc cancels); More menu owns global destructive actions (Clear all annotations with confirmation; Reset dock position retained, D-034 #6); mutations POST the last-known task through the existing atomic rewrite with a 300 ms debounce + flush on unload (keepalive) — an edit <300 ms before reload is persisted, not lost (found by the G03 e2e); an EMPTY annotations array is a VALID v5 task (clear-all; endpoint accepts it); deleted/hidden/edited state survives reload/routes/HMR and never resurrects | annotation-ops.test.ts (8), toolbar.test.tsx (27 incl. multi flow + action tests), endpoint empty-task test, e2e G03 scenario; evidence-annotation-first-g03/ |
| D-040 | G04 (Copy/Complete/shared formatter) | Status-update endpoint shape decision: NO new PATCH endpoint — Complete/Complete-all persist through the EXISTING atomic POST rewrite (same debounce+keepalive flush as G03 mutations), the safest minimal single-write-path; verify payload gains `completed` (isTaskCompleted over the updated task) and the verify CLI exits 0 for completed tasks without an edit wait, while open-task semantics (0 matched / 1 stale / 2 error) are unchanged; Copy/Complete/CLI/MCP all render through src/studio/format.ts (D-033 #15 single formatter, explicit .ts specifiers for Node 22 type stripping; normalizeTask widened to v1–v5 so the CLI keeps printing historical intermediates as normalized v5); the old Clear-task normal path is REMOVED (D-033 #13) — per-marker delete (G03) and the agent DELETE endpoint remain; completed markers render distinctly and survive reload/HMR | format-golden.test.ts (6), print-cli/mcp updated parity tests, toolbar 33 (Copy + Complete + Clear-task absent), endpoint completed preservation test, annotation-ops complete tests; evidence-annotation-first-g04/ |
| D-041 | G05 (hardening & cleanup) | Audit-discovered fixes + consolidations: (1) a11y — aria-pressed on hide/complete toggles, Tab focus containment for the More menu and the manual-copy dialog (Esc closes + focus returns), keyboard walkthrough e2e (shadowRoot.activeElement piercing); (2) theme — dark shadcn oklch unit case added (oklch parsing itself pre-existed), light/dark/no-vars hosts verified with screenshots; (3) i18n parity test (strict en/zh key-set equality) + hardcoded-string scan (zero); (4) lint zero errors AND zero warnings over src/studio/scripts/e2e/tests; (5) D-035 licensing scans archived (zero Agentation/Instruckt source hits — nothing copied, no MIT attribution needed); (6) docs consolidated — security-notes/usage-codex/usage-pi-json/mcp-config updated for v5 annotations, shared formatter, Copy/Complete, verify completed semantics; 08-annotation-first.md drift reconciled (no --schema v4 flag; status shape decided in D-040); (7) nb portal push/pull compatibility documented (D-042) and re-verified (push packs .portal-studio; pull wipes local annotations → restart dev); (8) G02 EOF blank-line items removed; (9) production exclusion re-verified (build + dist grep 0 + POST probes 404 on all seven endpoints) | evidence-annotation-first-g05/ (theme/a11y screenshots, lint-zero + licensing + parity logs, security tests, probes, gates) |
| D-042 | Annotation-first (compat reconciliation) | Plan reconciled against origin/develop @ f0a480e9f0 portal-source behavior: IMPACT 1 (HIGH, upstream gap) `nb portal push` packs `.portal-studio/` (pack filter excludes only .git/node_modules/dist/.DS_Store/._*) — session token + G02 annotations/screenshots would be pushed; NO plan code change (nocobase read-only), upstream filter addition recommended (`.portal-studio`, `test-results`), G05 documents + re-verifies; IMPACT 2 (MEDIUM) `nb portal pull` wipes `.portal-studio/` (git `clean -fdx` / dir replace) — annotations are dev-session-local, session regenerates on next dev, G05 documents "restart dev after pull"; IMPACT 3-5 NONE (#10315 identity, X-Portal/AI record, env/build html) | evidence-reconcile/05-plan-compat-impacts.md (filter executed against template root: .portal-studio -> PACKED, test-results -> PACKED) |
| D-043 | G05 acceptance (agent-browser) | REAL agent-browser acceptance found that mutation POSTs failed silently for tasks > 64 KB: sendMutation always set `keepalive: true`, and Chromium rejects keepalive bodies over 64 KB ("TypeError: Failed to fetch" — the artifact cap is 256 KB); the catch swallowed the error. Fix: the regular debounced mutation path sends a PLAIN POST (like saveTask); only the unload flush uses keepalive, guarded by KEEPALIVE_SAFE_BYTES = 60 KB (module-scoped, TextEncoder byte length) — oversized flushes warn and skip, losing at most an edit made < 300 ms before unload on a > 60 KB task (strictly better than silent failure). Regressions: component tests (no keepalive on the debounced POST; oversized flush warns + sends nothing) + e2e seeding a > 64 KB task, UI hide, reload, persistence asserted | commit 164466c; toolbar.test.tsx 35; e2e 12/12; evidence-annotation-first-agent-browser/ |
| D-044 | G05 acceptance (agent-browser) | REAL agent-browser acceptance found that numbered markers never resolved for elements whose FIRST class contains CSS-special characters: the bounded path candidate in collectSelectorCandidates was built with an UNESCAPED first class — Tailwind classes like `group/button` produced `button.group/button`, and `document.querySelector` throws a SyntaxError for the unescaped slash, so the marker overlay could never resolve that target (and agents received an unusable selector). Fix: the path part escapes the first class with the existing escapeCss helper (same as the id branch); the display-only domOutline is intentionally left unescaped. Regression: capture tests assert the escaped selector is valid and resolves, and that normal classes still produce plain paths | commit 3a67a2b; capture.test.ts 10; evidence-annotation-first-agent-browser/ |
